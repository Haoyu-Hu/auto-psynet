...
| `/apsy:project-dir` | **P0** | set or inspect `APSY_PROJECT_DIR` — the consistent root where new experiments are scaffolded. Default if unset: current working directory. Optionally invokes `bin/apsy-link-data.sh` to symlink `~/psynet-data` → `$APSY_PROJECT_DIR/data` (5-case safety table) to redirect PsyNet's hardcoded `assets`/`launch-data`/`artifacts` paths transparently. |
| `/apsy:services` | **P0** | start/stop/status of Redis + PostgreSQL for `psynet debug local`. Wraps `bin/apsy-services.sh`: detects binaries via PATH or common conda paths; `initdb`'s the pg data dir on first start; auto-creates the `dallinger` superuser + database; idempotent on already-running. Subcommands: `status` (default) / `start` / `stop` / `restart`. |
| `/apsy:doctor` | **P0** | environment validation — reports the resolved "apsy python" + source, delegates the dep/version section to `apsy-check`, checks Redis + Postgres reachable (HARD — required for `psynet debug local`; suggests `/apsy:services start` on fail), reports `APSY_PROJECT_DIR` writability, LLM key, AWS, config. |
| `/apsy:status` | **P0** | where am I (reads `<exp>/.apsy/state.json`) |
| `/apsy:help [name]` | **P0** | browse the command surface — no args lists every `/apsy:*` command + one-line description (sourced from each `commands/*.md`'s `description:` frontmatter); a name prints the full help for that command (`cat commands/<name>.md`); `--search <q>` filters by keyword. The user-facing discovery entry point — also the source of truth referenced by the (now-concise) top-level `README.md`. |
...
| `/apsy:debug` | **P0** | nohup-launch `psynet debug local` via `bin/apsy-debug.sh`: pre-flight auto-fixes (.gitignore / git init / constraints.txt), Redis+Postgres reachability checks (suggests `/apsy:services` on fail), soft-checks (`recruiter`/`dashboard_password`), lifecycle reminder, then exec → writes PID to `.apsy/runtime.pid`. `/apsy:debug stop` SIGINTs + sweeps orphan workers. |
| `/apsy:export` | **P0** | export data while `psynet debug local` is live. Wraps `bin/apsy-export.sh`: auto-passes `--path $APSY_PROJECT_DIR/data/<study>` when project-dir is set (else psynet default `~/psynet-data/export/`). Preflight checks the runtime is running; forwards extra args (`--no-source`, `--assets`, etc.). |
| `/apsy:pilot` | **P0** | run llm-pilot (+ `debug`), gate G3 |
| `/apsy:analyze` | **P0** | data-quality → analyze → interpret (operates on exported data; doesn't trigger the export itself — use `/apsy:export` first) |
