...
have redis-cli && { redis-cli ping >/dev/null 2>&1 && ok "redis reachable on localhost:6379 (required for debug local)" || bad "redis-cli installed but Redis is DOWN — easy fix: bash bin/apsy-services.sh start"; } || bad "redis-cli not found — Redis is REQUIRED for psynet debug local. Install: apt install redis-server (Ubuntu) · brew install redis (macOS) · conda -c conda-forge redis-server (HPC fallback). pip/uv can't install it — not a Python pkg. Once installed, run: bash bin/apsy-services.sh start"
have pg_isready && { pg_isready >/dev/null 2>&1 && ok "postgres reachable on localhost:5432 (required for debug local; also need dallinger user + dallinger db)" || bad "pg_isready installed but Postgres is DOWN — easy fix: bash bin/apsy-services.sh start (handles initdb + dallinger setup)"; } || bad "pg_isready not found — PostgreSQL is REQUIRED for psynet debug local. Install: apt install postgresql (Ubuntu) · brew install postgresql@14 (macOS) · conda -c conda-forge postgresql (HPC fallback). pip/uv can't install it. Once installed, run: bash bin/apsy-services.sh start"
if have docker; then
